...
function classify(tags: Record<string, string>): StrategicObjectType | null {
  if (tags.place === 'city' || tags.place === 'town') return tags.place === 'city' ? 'city' : 'city';
  if (tags.military === 'airfield' || tags.aeroway === 'aerodrome' || tags.aeroway === 'airport') return 'airport';
  if (tags.military === 'naval_base' || tags.military === 'naval_base') return 'naval-base';
  if (tags.military || tags.landuse === 'military') return 'military-base';
...
function confidence(type: StrategicObjectType, tags: Record<string, string>): ObjectConfidence {
  if (type === 'city' || tags.name || tags['name:en']) return 'high';
  return 'medium';
}
...
function buildQuery(south: number, west: number, north: number, east: number, includeTowns: boolean): string {
  ...
  return `[out:json][timeout:25];(
    ...
    nwr[military~"^(base|airfield|naval_base|barracks|range|danger_area)$"](${south},${west},${north},${east});
...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  const key = [south, west, north, east, Math.floor(zoom)].map((value) => Number(value).toFixed(2)).join(':');
  const cached = cache.get(key);
  if (cached && cached.expiresAt > Date.now()) return NextResponse.json({ objects: cached.objects, cached: true });
  const fetchedAt = new Date().toISOString();
  const localObjects = [...STRATEGIC_OBJECTS.filter((object) => object.latitude >= south! && object.latitude <= north! && object.longitude >= west! && object.longitude <= east!), ...getLocalPopulatedPlaces(south!, west!, north!, east!, zoom >= 5, fetchedAt)];
  if (zoom < 4) return NextResponse.json({ objects: localObjects, cached: true, sourceDate: new Date().toISOString() });

  const endpoint = process.env.OVERPASS_API_URL ?? 'https://overpass-api.de/api/interpreter';
  try {
    const query = buildQuery(south!, west!, north!, east!, zoom >= 5);
    ...
    if (!response.ok) return NextResponse.json({ objects: [], stale: true, error: `Overpass returned ${response.status}` });
    const payload = await response.json() as OverpassResponse;
    const remoteObjects = (payload.elements ?? []).map((element, index) => mapElement(element, index, fetchedAt)).filter((object): object is StrategicObject => object !== null);
    ...
    return NextResponse.json({ objects, cached: false, sourceDate: fetchedAt });
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : 'Object source unavailable';
    return NextResponse.json({ objects: localObjects, stale: true, error: message }, { status: 200 });
  }
}
